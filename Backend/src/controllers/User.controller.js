...
export async function sendFriendRequest(req,res){
    try {
       ...
       res.status(201).json(friendRequest);
        ...
        console.error("Error im sendFriendRequest controller", error.message);
        res.status(500).json({message:"Internal Server Error"});
    }
...
export async  function acceptFriendRequest(req,res){
    try {
        ...
        console.log("Error in acceptedFriendRequest constroller", error.message);
        res.status(500).json({message:"Internal Server Error"});
    }
}
...
export async function getOutgoingFriendRegs(req,res){
    try {
       ...
       res.status(200).json(outgoingRequets)
        ...
        console.log("Error in getOutgoingFriendReqgs controller", error.message)
        res.status(500).json({message:"Internal Server Error"})
    }
}